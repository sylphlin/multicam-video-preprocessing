...
3. **階段三（局部音軌 + 全篇名詞庫之 Gemini 音訊多模態精修審稿）**：
   - 將全片切為 2–3 分鐘區塊，Gemini 邊聽局部真實音訊、邊參照全篇詞彙庫，在**嚴格鎖定時間戳與行號**的前提下，修復同音錯字（如「矽谷」、「薪水」、「Buffet」）與英文專有名詞（如 `Kelly Tsai`、`YouTube`、`DaVinci Resolve`）。
4. **輸出檔案**：
