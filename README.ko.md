...
1. **8kHz FFT 오디오 타임라인 동기화**:
   - 오디오를 8kHz 모노로 다운샘플링하고 상호상관 알고리즘으로 시간 편차 $\Delta t$(밀리초 정밀도)를 계산.
2. **EBU R128 (-14 LUFS) 전체 음량 표준화**:
   - 2패스 라우드니스 분석을 통해 모든 카메라 트랙을 -14.0 LUFS, 11.0 LRA, -1.5 dBTP로 표준화.
3. **30–40분 자연스러운 멈춤 챕터 분할 (Auto-Split)**:
   - 30–40분 목표 구간 내에서 음성 에너지 최소 지점을 감지하여 무손실 분할.
